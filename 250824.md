...
- 🤔 에피소드
<br>: 디자인패턴 스터디 중에 기존 인스턴스를 복제하여 새로운 인스턴스를 만들 때는
<br>얕은 복사를 해서 필드값이 그대로 복사되기 때문에 깊은복사라는 이야기가 나왔는데
<br>책의 뒷 페이지에 얕은복사라고 표기되어 있었다. 그래서 얕은복사와 깊은복사가 무엇인지 궁금해져서 알아보았다.
